...
router.post("/login", async (req, res) => {
  try {
    ...
    await db.execute("UPDATE vendors SET last_login = NOW() WHERE id = ?", [
      vendor.id,
    ]);
...
router.post("/logout", async (req, res) => {
  try {
    ...
    await db.execute("DELETE FROM vendor_sessions WHERE vendor_id = ?", [
      decoded.id,
    ]);
...
router.put("/change-password", async (req, res) => {
  try {
    ...
    // Atualizar senha
    await db.execute("UPDATE vendors SET password = ? WHERE id = ?", [
      hashedPassword,
      decoded.id,
    ]);
