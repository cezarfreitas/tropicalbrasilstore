...
export async function downloadAndSaveImage(imageUrl: string, productSku: string, colorName: string): Promise<string | null> {
  try {
    ...
    const safeProductSku = productSku.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase();
