...
async function testConnection(retries = 3): Promise<boolean> {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      isConnected = true;
      console.log(`✅ Conexão com banco de dados estabelecida (tentativa ${i + 1})`);
      return true;
    } catch (error) {
      console.warn(`⚠️  Tentativa ${i + 1}/${retries} de conectar falhou:`, error.message);
...
export async function getDatabase(): Promise<{ db: any; isConnected: boolean }> {
