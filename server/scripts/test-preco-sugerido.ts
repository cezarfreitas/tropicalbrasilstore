...
async function testPrecoSugerido() {
  ...
  try {
    ...
    console.log(
      `   • Variantes criadas: ${response.data.data.variantes_novas}`,
    );
    console.log("");
    ...
    if ((product as any[]).length > 0) {
      ...
      console.log(
        `   • Venda Infinita: ${prod.sell_without_stock ? "SIM" : "NÃO"}`,
      );
    } else {
    ...
    const adminProduct = adminResponse.data.data.find(
      (p: any) => p.sku === "PRECO001",
    );
    ...
    if (adminProduct) {
      ...
      console.log(
        `   • Venda Infinita: ${adminProduct.sell_without_stock ? "SIM" : "NÃO"}`,
      );
    } else {
    ...
    console.log(
      `   • Produtos atualizados: ${updateResponse.data.data.produtos_atualizados}`,
    );
    console.log(
      `   • Variantes novas: ${updateResponse.data.data.variantes_novas}`,
    );
    console.log("");
