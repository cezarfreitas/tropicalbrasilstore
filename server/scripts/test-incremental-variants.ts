...
async function testIncrementalVariants() {
  ...
  try {
    const response1 = await axios.post(`${API_BASE}/products/bulk`, firstRequest, {
      headers: {
    ...
    const response2 = await axios.post(`${API_BASE}/products/bulk`, secondRequest, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${API_KEY}`,
      },
    ...
    console.log("- Produtos atualizados:", response2.data.data.produtos_atualizados);
    console.log("- Variantes novas:", response2.data.data.variantes_novas);
    console.log("- Variantes existentes:", response2.data.data.variantes_existentes);
    console.log("");

    // 3. Terceira requisição - tentar adicionar variante azul novamente (deve detectar existente)
    console.log("📤 3. Tentando adicionar variante Azul novamente (deve detectar existente)...");
    ...
    const response3 = await axios.post(`${API_BASE}/products/bulk`, thirdRequest, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${API_KEY}`,
      },
    ...
    console.log("- Produtos atualizados:", response3.data.data.produtos_atualizados);
    console.log("- Variantes novas:", response3.data.data.variantes_novas);
    console.log("- Variantes existentes:", response3.data.data.variantes_existentes);
    console.log("");
    ...
    console.log("Total de variantes de cor:", (productVariants as any[]).length);
    ...
    const incProduct = wooResponse.data.data.find((p: any) => p.sku === "INC001");
    if (incProduct) {
