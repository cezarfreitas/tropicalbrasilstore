...
async function checkGradeStructure() {
  try {
    ...
    try {
      const [colorGradeColumns] = await db.execute('DESCRIBE product_color_grades');
    ...
    const [gradeCount] = await db.execute('SELECT COUNT(*) as count FROM grade_vendida');
    console.log(`- Grades: ${(gradeCount as any[])[0].count}`);
    
    const [templateCount] = await db.execute('SELECT COUNT(*) as count FROM grade_templates');
    console.log(`- Templates: ${(templateCount as any[])[0].count}`);
    
    const [colorGradeCount] = await db.execute('SELECT COUNT(*) as count FROM product_color_grades');
    console.log(`- Associações produto-cor-grade: ${(colorGradeCount as any[])[0].count}`);
