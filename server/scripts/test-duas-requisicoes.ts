...
async function testDuasRequisicoes() {
  ...
  try {
    ...
    console.log(
      `   • Produtos atualizados: ${resposta1.data.data.produtos_atualizados}`,
    );
    console.log(`   • Variantes novas: ${resposta1.data.data.variantes_novas}`);
    console.log(
      `   • Variantes existentes: ${resposta1.data.data.variantes_existentes}`,
    );
    ...
    // ========================================
    console.log(
      "📤 2ª REQUISIÇÃO - Adicionando variante AZUL ao produto existente...",
    );
    ...
    console.log(
      `   • Produtos atualizados: ${resposta2.data.data.produtos_atualizados}`,
    );
    console.log(`   • Variantes novas: ${resposta2.data.data.variantes_novas}`);
    console.log(
      `   • Variantes existentes: ${resposta2.data.data.variantes_existentes}`,
    );
    ...
    // ========================================
    console.log(
      "🔍 VERIFICAÇÃO FINAL - Estado do produto na interface admin...",
    );

    const verificacao = await axios.get(`${API_BASE}/products-woocommerce`);
    const produtoTeste = verificacao.data.data.find(
      (p: any) => p.sku === "TESTE001",
    );
    ...
    if (produtoTeste) {
      ...
      console.log(
        `   • Vender infinito: ${produtoTeste.sell_without_stock ? "SIM" : "NÃO"}`,
      );
    ...
    console.log(
      "   ✅ 2ª requisição adicionou variante azul ao produto existente",
    );
    console.log(
      "   ✅ Produto aparece na interface admin com 2 variantes de cor",
    );
  } catch (error: any) {
