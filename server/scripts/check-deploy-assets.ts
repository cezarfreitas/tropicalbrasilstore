...
async function checkDeployAssets() {
  try {
    ...
    const [settings] = await db.execute(
      "SELECT store_name, logo_url FROM store_settings LIMIT 1",
    );
    ...
    if ((settings as any[]).length > 0) {
      ...
      console.log(
        `🖼️ Logo URL: ${setting.logo_url || "Nenhum logo configurado"}`,
      );
      ...
      if (setting.logo_url) {
        ...
        console.log(
          `📁 Arquivo local: ${localExists ? "✅ Existe" : "❌ Não existe"}`,
        );
    ...
    console.log(
      "❌ O Dockerfile não copia o diretório public/uploads/ para produção",
    );
