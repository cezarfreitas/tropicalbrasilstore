...
async function fixAcentuacao() {
  try {
    ...
    console.log(
      "Produtos com possíveis problemas de acentuação:",
      produtosSemAcentuacao,
    );
    ...
    console.log(
      "   - client/pages/Categories.tsx (usar 'Descrição', 'Categorias')",
    );
    console.log("   - client/pages/CustomerOrders.tsx (usar 'Histórico')");
    console.log(
      "   - server/routes/customer-auth.ts (usar 'obrigatórios', 'dígitos')",
    );

    console.log("\n⚠️ Problemas encontrados:");
    console.log(
      "   - server/routes/products.ts: 'preco' -> deve usar 'preço' em comentários",
    );
    console.log(
      "   - Variáveis mantêm snake_case para compatibilidade com banco",
    );
    console.log("   - Interfaces TypeScript usam 'preco' por compatibilidade");

    console.log("\n🎯 Recomendações:");
    console.log(
      "   1. Manter nomes de variáveis/campos em inglês ou sem acentos",
    );
