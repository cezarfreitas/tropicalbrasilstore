async function testApiLogs() {
  try {
    ...
    const response3 = await fetch(
      "http://localhost:8080/api/admin/nonexistent",
    );
    ...
    if (logsResponse.ok) {
      ...
      logsData.logs.slice(0, 3).forEach((log: any, index: number) => {
        console.log(
          `  ${index + 1}. ${log.method} ${log.endpoint} - Status: ${log.response_status} - Tempo: ${log.response_time_ms}ms`,
        );
      });
    } else {
    ...
    if (statsResponse.ok) {
      ...
      console.log(
        `  Total de requisições: ${statsData.overview.total_requests}`,
      );
      console.log(
        `  Requisições de sucesso: ${statsData.overview.success_requests}`,
      );
      console.log(
        `  Requisições com erro: ${statsData.overview.error_requests}`,
      );
      console.log(
        `  Tempo médio: ${Math.round(statsData.overview.avg_response_time)}ms`,
      );

      console.log("\n📋 Top endpoints:");
      statsData.top_endpoints
        .slice(0, 3)
        .forEach((endpoint: any, index: number) => {
          console.log(
            `  ${index + 1}. ${endpoint.method} ${endpoint.endpoint} - ${endpoint.request_count} requisições`,
          );
        });
    } else {
    ...
    console.log(
      "💡 Acesse /admin/api e vá na aba 'Logs de API' para ver a interface completa",
    );
  } catch (error) {
