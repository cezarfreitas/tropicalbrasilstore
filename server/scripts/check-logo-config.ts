...
async function checkLogoConfig() {
  try {
    ...
    console.log("\n🖼️ Logo URL atual:");
    const [logoData] = await db.execute('SELECT id, store_name, logo_url FROM store_settings LIMIT 1');
    ...
    if ((logoData as any[]).length > 0) {
      ...
      console.log("❌ Nenhuma configuração encontrada na tabela store_settings");
    }
