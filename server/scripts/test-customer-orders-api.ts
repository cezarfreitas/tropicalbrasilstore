  ...
  try {
    ...
    if (response1.ok) {
      ...
      orders1.forEach((order: any, index: number) => {
        console.log(
          `  📦 Pedido ${index + 1}: ${order.id} - Status: ${order.status} - Total: R$ ${order.total} - Itens: ${order.items.length}`,
        );
      });
    } else {
    ...
    const response2 = await fetch(
      "http://localhost:8080/api/customer-orders/by-email/maria.silva@email.com",
    );
    ...
    if (response2.ok) {
      ...
      orders2.forEach((order: any, index: number) => {
        console.log(
          `  📦 Pedido ${index + 1}: ${order.id} - Status: ${order.status} - Total: R$ ${order.total}`,
        );
        order.items.forEach((item: any, itemIndex: number) => {
          console.log(
            `    🛒 Item ${itemIndex + 1}: ${item.productName} - Qtd: ${item.quantity} - Preço: R$ ${item.unitPrice}`,
          );
        });
      });
    } else {
    ...
    const response3 = await fetch(
      "http://localhost:8080/api/customer-orders/9999",
    );
    ...
    if (!response3.ok) {
      ...
      console.log(
        `❌ Deveria ter retornado erro, mas retornou: ${JSON.stringify(orders3)}`,
      );
    }
