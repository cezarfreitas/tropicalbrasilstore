...
async function checkApiLogsInDatabase() {
  try {
    ...
    const [countResult] = await db.execute('SELECT COUNT(*) as total FROM api_logs');
    ...
    try {
      ...
      const [newCount] = await db.execute('SELECT COUNT(*) as total FROM api_logs');
