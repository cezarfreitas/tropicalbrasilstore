async function testAdminApiLogsOnly() {
  try {
    ...
    // Contar logs atuais
    const initialResponse = await fetch('http://localhost:8080/api/admin/logs/stats');
    ...
    const adminOrdersResponse = await fetch('http://localhost:8080/api/admin/orders');
    console.log(`GET /admin/orders: ${adminOrdersResponse.status} (não deve ser logado)`);
    
    // Rota /admin/customers (não deve logar)
    const adminCustomersResponse = await fetch('http://localhost:8080/api/admin/customers');
    console.log(`GET /admin/customers: ${adminCustomersResponse.status} (não deve ser logado)`);
    ...
    const finalResponse = await fetch('http://localhost:8080/api/admin/logs/stats');
    ...
    if (newLogs === 1) {
      console.log("✅ CORRETO: Apenas 1 nova requisição foi logada (/admin/api)");
    } else if (newLogs === 0) {
      console.log("⚠️ Nenhum log novo (pode ser que /admin/api não exista)");
    } else {
      console.log(`❌ ERRO: ${newLogs} requisições foram logadas (deveria ser apenas 1)`);
    }

    // 4. Mostrar últimos logs para confirmar
    console.log("\n📋 Últimos logs registrados:");
    const logsResponse = await fetch('http://localhost:8080/api/admin/logs?limit=3');
    if (logsResponse.ok) {
      const logsData = await logsResponse.json();
      logsData.logs.forEach((log: any, index: number) => {
        console.log(`  ${index + 1}. ${log.method} ${log.endpoint} - ${new Date(log.created_at).toLocaleTimeString()}`);
      });
    }

    console.log("\n🎯 RESULTADO:");
    console.log("- ✅ Rotas /admin/orders e /admin/customers NÃO devem aparecer nos logs");
