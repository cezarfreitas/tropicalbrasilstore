async function testCompleteFlow() {
  try {
    ...
    if (response1.ok) {
      ...
      const response2 = await fetch(`http://localhost:8080/api/customer-orders/${customerData.id}`);
      ...
      if (response2.ok) {
        ...
        orders.forEach((order: any, index: number) => {
          ...
          console.log(`  📅 Data: ${new Date(order.date).toLocaleDateString('pt-BR')}`);
          ...
          order.items.forEach((item: any, itemIndex: number) => {
            ...
            console.log(`      - Preço unitário: R$ ${item.unitPrice.toFixed(2)}`);
            if (item.colorName) console.log(`      - Cor: ${item.colorName}`);
            if (item.gradeName) console.log(`      - Grade/Tamanho: ${item.gradeName}`);
          });
        });
        ...
        const frontendResponse = await fetch(`http://localhost:8080/api/customer-orders/${customerData.id}`, {
          headers: {
        ...
        if (frontendResponse.ok) {
          ...
          console.log(`✅ Frontend pode acessar ${frontendOrders.length} pedidos`);
        } else {
          console.log(`❌ Erro na requisição do frontend: ${frontendResponse.status}`);
        }
      ...
      const customerResponse = await fetch('http://localhost:8080/api/admin/customers');
