...
async function createTestOrders() {
  try {
    ...
    if ((existingCustomers as any[]).length === 0) {
      ...
      console.log(
        `✅ Cliente criado: ID ${customerId}, Email: ${customerEmail}`,
      );
    } else {
      customerId = (existingCustomers as any[])[0].id;
      customerEmail = (existingCustomers as any[])[0].email;
      console.log(
        `✅ Usando cliente existente: ID ${customerId}, Email: ${customerEmail}`,
      );
    }
    ...
    console.log(
      `📦 Encontrados ${(existingProducts as any[]).length} produtos`,
    );
    ...
    const orders = [
      ...
      },
      {
        ...
        items: [
          {
            product_id: (existingProducts as any[])[1]
              ? (existingProducts as any[])[1].id
              : (existingProducts as any[])[0].id,
            quantity: 1,
            unit_price: 299.8,
            type: "individual",
          },
      ...
      },
      {
        ...
        items: [
          {
            product_id: (existingProducts as any[])[2]
              ? (existingProducts as any[])[2].id
              : (existingProducts as any[])[0].id,
            quantity: 1,
            unit_price: 89.9,
            type: "individual",
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      console.log(
        `📝 Pedido criado: ID ${orderId}, Status: ${order.status}, Total: R$ ${order.total}`,
      );
      ...
      for (const item of order.items) {
        ...
        console.log(
          `  ✅ Item adicionado: Produto ${item.product_id}, Qtd: ${item.quantity}, Preço: R$ ${item.unit_price}`,
        );
      }
    }
    ...
    console.log(
      `📊 Resultado: ${(verifyOrders as any[])[0].total} pedidos, ${(verifyItems as any[])[0].total} itens criados`,
    );
