...
async function fixGradeQuantities() {
  try {
    ...
    for (const update of correctQuantities) {
      ...
      console.log(`  ✅ Tamanho ${update.size_id}: ${update.quantity} unidades`);
    }
    ...
    // 3. Calcular total
    const totalQuantity = correctQuantities.reduce((sum, item) => sum + item.quantity, 0);
    ...
    if (response.ok) {
      ...
      if (product.available_grades && product.available_grades.length > 0) {
        ...
        product.available_grades.forEach((grade: any, index: number) => {
          console.log(`  ${index + 1}. ${grade.name} - Cor: ${grade.color_name}`);
          console.log(`     Total: ${grade.total_quantity} peças`);
          console.log(`     Estoque completo: ${grade.has_full_stock ? 'SIM' : 'NÃO'}`);
        });

        console.log("\n💡 Agora na loja aparecerá:");
        console.log("   ✅ 'Selecione a Grade' em vez de 'Selecione o Tamanho'");
        console.log("   ✅ Duas opções de grade (uma para cada cor)");
        console.log("   ✅ Cada grade mostra os tamanhos e quantidades");
      } else {
        console.log("❌ Ainda não há grades disponíveis - verificar condições da API");
      }
    } else {
