  ...
  try {
    ...
    if (product.available_colors && product.available_colors.length > 0) {
      ...
      product.available_colors.forEach((color: any, index: number) => {
        console.log(`  ${index + 1}. ${color.name} (ID: ${color.id}) - Hex: ${color.hex_code}`);
      });
    } else {
    ...
    if (product.variants && product.variants.length > 0) {
      ...
      product.variants.forEach((variant: any, index: number) => {
        console.log(`  ${index + 1}. Cor: ${variant.color_name}, Tamanho: ${variant.size}, Estoque: ${variant.stock}`);
      });
    } else {
    ...
    if (product.available_grades && product.available_grades.length > 0) {
      ...
      product.available_grades.forEach((grade: any, index: number) => {
        console.log(`  ${index + 1}. ${grade.name} - Cor ID: ${grade.color_id}, Tem estoque completo: ${grade.has_full_stock}`);
        if (grade.templates) {
          console.log(`     Templates: ${grade.templates.length} tamanhos`);
          grade.templates.forEach((template: any) => {
            console.log(`       - ${template.size}: ${template.required_quantity} peças`);
          });
        }
      });
    } else {
    ...
    console.log(`- Produto permite venda sem estoque: ${product.sell_without_stock ? 'SIM' : 'NÃO'}`);
    
    if (product.sell_without_stock) {
      console.log("- ✅ Como tem venda infinita, DEVERIA mostrar todas as variantes/grades");
