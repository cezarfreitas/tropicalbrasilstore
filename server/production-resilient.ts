...
async function initializeWithRetry() {
  ...
  while (retryCount < maxRetries) {
    try {
      console.log(`🔄 Tentativa ${retryCount + 1}/${maxRetries} de conectar com o banco...`);
  ...
  console.log("❌ Não foi possível conectar com o banco após todas as tentativas");
...
async function startServer() {
  ...
  try {
    ...
    if (fullApp) {
      ...
      fullApp.use("/uploads", express.static(path.join(process.cwd(), "public", "uploads")));
