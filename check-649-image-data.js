...
async function check649ImageData() {
  ...
  try {
    ...
    colorVariants.forEach((variant, index) => {
      ...
      console.log(`     Color: ${variant.color_name} (${variant.hex_code || 'no hex'})`);
    ...
    console.log(`Variants with images: ${colorVariants.filter(v => v.image_url).length}`);
    ...
    if (storeApiResult.length > 0 && listingApiResult.length === 0) {
      ...
      console.log("This explains why image appears in product page but not in card.");
    }
