...
async function testStoreAPI() {
  ...
  try {
    const response = await fetch('http://localhost:5000/api/store/products-paginated?page=1&limit=5');
    ...
    if (response.ok) {
      ...
      if (data.products && data.products.length > 0) {
        ...
        data.products.forEach((product, index) => {
          ...
          console.log(`  - available_colors: ${product.available_colors?.length || 0} colors`);
