...
const CORE_TABLES = [
  'products', 'categories', 'colors', 'sizes', 'brands', 'types', 'genders',
  'product_variants', 'product_color_variants', 'product_variant_sizes', 'variant_images',
  'grade_vendida', 'grade_templates', 'product_color_grades',
  'customers', 'customer_auth', 'orders', 'order_items',
  'vendors', 'vendor_commissions', 'vendor_sessions',
  'store_settings', 'size_groups', 'api_logs', 'notification_settings'
];
...
async function removeUnusedTables() {
  ...
  try {
    ...
    const unrecognizedTables = allTables.filter(table => 
      ...
      for (const { table, count } of obsoleteTablesFound) {
        ...
        try {
          ...
          if (count > 0) {
            ...
            console.log(`  💾 Backup seria salvo em: backup_${table}_${new Date().toISOString().split('T')[0]}.json`);
