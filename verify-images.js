...
async function verifyImages() {
  try {
    ...
    for (const product of products) {
      const relativePath = product.photo.startsWith('/') ? product.photo.substring(1) : product.photo;
      const fullPath = join(process.cwd(), 'public', relativePath);
      const exists = existsSync(fullPath);
      
      console.log(`  ${product.name}: ${product.photo} - ${exists ? '✅ EXISTS' : '❌ MISSING'}`);
    }
    ...
    for (const variant of variants) {
      const relativePath = variant.image_url.startsWith('/') ? variant.image_url.substring(1) : variant.image_url;
      const fullPath = join(process.cwd(), 'public', relativePath);
      const exists = existsSync(fullPath);
      
      console.log(`  ${variant.product_name} - ${variant.variant_name}: ${variant.image_url} - ${exists ? '✅ EXISTS' : '❌ MISSING'}`);
    }
