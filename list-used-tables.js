...
const usedTables = {
  // Produtos e Catálogo
  'products': ['server/routes/products.ts', 'server/routes/store-simple.ts', 'server/routes/store.ts'],
  ...
  'types': ['server/lib/add-genders-types-tables.ts', 'server/routes/store-simple.ts'],
  'genders': ['server/lib/add-genders-types-tables.ts', 'server/routes/store-simple.ts'],
  
  // Sistema de Variantes
  'product_variants': ['server/routes/store-simple.ts', 'server/routes/products.ts'],
  'product_color_variants': ['server/routes/store-simple.ts', 'server/routes/products.ts'],
  ...
  'product_color_grades': ['server/routes/store-simple.ts', 'server/lib/add-grades.ts'],
  
  // Loja e Vendas
  'customers': ['server/routes/customer-auth.ts', 'server/routes/admin-customers.ts'],
  ...
  'vendor_commissions': ['server/routes/vendors.ts', 'server/lib/add-vendor-commissions.ts'],
...
console.log("📝 RESUMO:");
console.log("=".repeat(50));
console.log(`• Tabelas identificadas como utilizadas: ${Object.keys(usedTables).length}`);
console.log(`• Tabelas possivelmente não utilizadas: ${potentiallyUnusedTables.length}`);
...
console.log("\n🔍 PRÓXIMOS PASSOS:");
console.log("1. Execute 'SHOW TABLES' no banco para verificar se há tabelas não listadas");
console.log("2. Verifique se há tabelas de backup, teste ou temporárias");
console.log("3. Analise tabelas com nomes como 'temp_', 'test_', 'backup_', etc.");
console.log("4. Verifique logs de aplicação para tabelas que podem ter sido criadas dinamicamente");
