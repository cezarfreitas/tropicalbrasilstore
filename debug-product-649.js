...
async function debugProduct649() {
  ...
  try {
    ...
    console.log(
      `✅ Store API responded with ${storeData.products?.length || 0} products`,
    );
    ...
    if (product649) {
      ...
      if (
        ...
        product649.available_colors.forEach((color, index) => {
          ...
          console.log(
            `  - Image URL trimmed: "${(color.image_url || "").trim()}"`,
          );
          console.log(
            `  - Has valid image: ${!!(color.image_url && color.image_url.trim())}`,
          );
        });
      } else {
      ...
      console.log(
        `  - Has valid photo: ${!!(product649.photo && product649.photo.trim())}`,
      );
    } else {
      console.log("❌ Product 649 not found in store API response!");
      console.log(
        "Available product IDs:",
        storeData.products?.map((p) => p.id) || [],
      );
    }
    ...
    try {
      const directResponse = await fetch(
        "http://localhost:3001/api/products/649",
      );
