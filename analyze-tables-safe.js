...
const OBSOLETE_TABLES = {
  'grade_items': 'OBSOLETA - Substituída por grade_templates + product_color_grades'
};
...
async function analyzeTables() {
  ...
  try {
    ...
    for (const tableName of activeTables) {
      ...
      console.log(`  📋 ${tableName.padEnd(25)} │ ${count.toString().padStart(8)} registros │ ${ACTIVE_TABLES[tableName]}`);
    }
    ...
    if (obsoleteTables.length > 0) {
      ...
      for (const tableName of obsoleteTables) {
        try {
          const [countResult] = await connection.execute(`SELECT COUNT(*) as count FROM \`${tableName}\``);
          const count = countResult[0].count;
          console.log(`  🗑️  ${tableName.padEnd(25)} │ ${count.toString().padStart(8)} registros │ ${OBSOLETE_TABLES[tableName]}`);
        } catch (error) {
          console.log(`  🗑️  ${tableName.padEnd(25)} │     ERRO │ ${OBSOLETE_TABLES[tableName]}`);
        }
      }
    }
    ...
    if (unknownTables.length > 0) {
      ...
      for (const tableName of unknownTables) {
        try {
          ...
          } else if (tableName.includes('backup') || tableName.includes('bkp')) {
          ...
          console.log(`  ❓ ${tableName.padEnd(25)} │ ${count.toString().padStart(8)} registros │ ${possibleType}`);
        } catch (error) {
          console.log(`  ❓ ${tableName.padEnd(25)} │     ERRO │ Erro ao acessar`);
        }
      }
    }
