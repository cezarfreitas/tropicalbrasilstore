...
async function checkProduct649() {
  ...
  try {
    ...
    const response = await fetch('http://localhost:5000/api/store/products-paginated?page=1&limit=50');
    ...
    if (response.ok) {
      ...
      if (product649) {
        ...
        console.log('  - Available colors:', product649.available_colors?.length || 0);
        console.log('  - Active:', product649.active);
        
        if (product649.available_colors) {
          product649.available_colors.forEach((color, index) => {
            console.log(`    Color ${index + 1}: ${color.name} - image_url: ${color.image_url || 'null'}`);
          });
        }
      } else {
        ...
        data.products?.forEach((p, index) => {
          console.log(`  ${index + 1}. ID: ${p.id} - ${p.name} - photo: ${p.photo || 'null'}`);
        });
      }
    } else {
    ...
    const productResponse = await fetch('http://localhost:5000/api/store/products/649');
