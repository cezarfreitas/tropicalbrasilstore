...
export const MinimumOrderIndicator: React.FC<MinimumOrderIndicatorProps> = ({
  ...
  const minimumValue =
    safeCustomerMinimum > 0 ? safeCustomerMinimum : safeGlobalMinimum;
  ...
  const containerClass = theme === "sidebar" ? "p-1 mb-1" : "p-3 mb-3";

  return (
    <div className={containerClass}>
      {/* Valores separados */}
      <div
        className={`flex justify-between text-xs mb-1 ${theme === "sidebar" ? "text-white/90" : "text-gray-700"}`}
      >
      ...
      <div
        className={`w-full rounded-full h-1 mb-1 overflow-hidden ${theme === "sidebar" ? "bg-white/20" : "bg-gray-200"}`}
      >
        <div
      ...
      {!isCompleted && (
        <div
          className={`text-[10px] ${theme === "sidebar" ? "text-white/60" : "text-gray-500"}`}
        >
