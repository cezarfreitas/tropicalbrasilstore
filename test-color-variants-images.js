...
async function testColorVariantsAPI() {
  try {
    ...
    console.log(`✅ Store API responded with ${data.products?.length || 0} products`);
    ...
    if (data.products && data.products.length > 0) {
      ...
      data.products.forEach((product, index) => {
        ...
        if (product.available_colors && product.available_colors.length > 0) {
          console.log(`  🎨 Available colors (${product.available_colors.length}):`);
          
          product.available_colors.forEach((color, colorIndex) => {
            const hasImage = color.image_url && color.image_url.trim() !== '';
            console.log(`    ${colorIndex + 1}. ${color.name} (${color.hex_code || 'no hex'})`);
            console.log(`       🖼️  image_url: ${color.image_url || 'NULL'} ${hasImage ? '✅' : '❌'}`);
          ...
          console.log(`  ❌ No available_colors (no product_color_variants data)`);
        }
      });

      // Find products with color variant images
      const productsWithImages = data.products.filter(p => 
        p.available_colors?.some(c => c.image_url && c.image_url.trim() !== '')
      );

      console.log(`\n📊 Summary:`);
      console.log(`- Total products: ${data.products.length}`);
      console.log(`- Products with available_colors: ${data.products.filter(p => p.available_colors?.length > 0).length}`);
      console.log(`- Products with color variant images: ${productsWithImages.length}`);

      if (productsWithImages.length > 0) {
        console.log(`\n🎯 Best products for testing SimpleProductCard:`);
        productsWithImages.slice(0, 3).forEach(product => {
          const firstImageColor = product.available_colors.find(c => c.image_url);
          console.log(`- Product ${product.id}: ${product.name}`);
          console.log(`  First image: ${firstImageColor.name} -> ${firstImageColor.image_url}`);
        });
      } else {
        console.log(`\n⚠️  No products found with color variant images!`);
        console.log(`This means the product_color_variants.image_url column is empty or the API isn't joining correctly.`);
      }
