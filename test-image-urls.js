...
async function testImageUrls() {
  try {
    ...
    const storeResponse = await axios.get(
      "https://ide-b2b-tb.jzo3qo.easypanel.host/api/store/products-paginated?page=1&limit=5",
    );
      ...
      storeResponse.data.products.forEach((product, index) => {
        ...
        if (product.available_colors) {
          product.available_colors.forEach((color, colorIndex) => {
            console.log(
              `   Color ${colorIndex + 1}: ${color.name} - Image: ${color.image_url || "null"}`,
            );
          });
        }
        console.log("");
    ...
    const wooResponse = await axios.get(
      "https://ide-b2b-tb.jzo3qo.easypanel.host/api/products-woocommerce?page=1&limit=5",
    );
      ...
      wooResponse.data.data.forEach((product, index) => {
        ...
        if (product.color_variants) {
          product.color_variants.forEach((variant, variantIndex) => {
            console.log(
              `   Variant ${variantIndex + 1}: ${variant.color_name} - Image: ${variant.image_url || "null"}`,
            );
          });
        }
        console.log("");
    ...
    try {
      const product649 = await axios.get(
        "https://ide-b2b-tb.jzo3qo.easypanel.host/api/store/products-paginated?page=1&limit=100",
      );
